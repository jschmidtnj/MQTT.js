...
function connect (brokerUrl, opts) {
  ...
  // the specified protocol is not supported by the current environment.
  if (!protocols[opts.protocol]) {
    var isSecure = ['mqtts', 'wss', 'quic'].indexOf(opts.protocol) !== -1
    // will update the protocol to the protocol that is supported.
    // If a secure protocol is requested, then will default to a supported
    // secure protocol.
    if (isSecure) {
      opts.protocol = [
        'mqtts',
